...
## v25 업데이트 — 온보딩 이미지 재구성: 제목·n/5 배지를 실시간 텍스트로, 본문은 실제 앱 화면 캡처로 교체

### 배경
v24에서 "캔버스 비율 통일"로 배지 위치 어긋남을 고쳤다고 기록했지만, 실사용
환경(하드 리프레시 포함)에서 여전히 어긋나 보인다는 피드백을 받았다. 픽셀
단위로 재측정한 결과, 진짜 원인은 캔버스 비율이 아니라 **5장 각각의 이미지
안에서 배지가 그려진 위치 자체가 미묘하게 달랐다**는 것(상단 오프셋
0.54%~2.16%p 편차) — v24의 캔버스 패딩 방식으로는 근본적으로 고칠 수 없는
종류의 문제였다.

### 근본 수정
"이미지에 텍스트를 굽는" 접근 자체를 버렸다. 이제:
- 온보딩 1~5단계: 제목·"건너뛰기"·"n/5" 배지를 전부 `OnboardingPage.tsx`의
  실시간 DOM(`.onboarding-head`)으로 렌더링하고, 그 아래(`.onboarding-photo`)에
  **실제 KOOK 앱을 프로덕션(kook-hemodialysis-meal-ai.vercel.app)에서 직접
  캡처한 화면 스크린샷**을 채운다. 이미지에는 어떤 텍스트도 없다 — 이미지를
  통째로 새로 바꿔도 배지·제목 위치는 항상 CSS로 고정된 같은 자리에 있다.
- 스플래시(`onboarding-home.png`)도 동일 원칙: 그림 속에 그려져 있던
  "시작하기" 버튼과, 그 위치를 %로 실측해 겹쳐 두던 투명 버튼(`.splash-hit`)을
  전부 제거하고 Shell의 실제 footer 버튼 하나로 통일. 이미지는 순수 일러스트만
  담당(사용자가 새로 그려서 교체).
- 온보딩 1~5단계 화면 캡처는 Chrome DevTools Protocol을 직접 구동해
  자동화했다: 실제 검색→식단 생성→영양 판정→레시피 재구성(AI 편집 조리법
  포함)→완성까지 클릭해서 이동하며 각 단계를 캡처. 5단계는 최종 영양 요약
  대신 "재료+AI 편집 조리과정"이 보이는 화면으로 캡처해 3단계(판정)와
  내용이 겹치지 않게 했다.
- 부수적으로 발견한 버그: `BackHeader`가 쓰는 `.detail-header{background:#fff}`
  가 뒤에 나오는 `.header,.footer{background:rgba(251,250,246,.96)}`에
  동일 우선순위로 덮어써져 헤더가 96%만 불투명했다(스크롤 중인 콘텐츠가
  아주 살짝 비쳐 보일 수 있음). `.header.detail-header`로 선택자를 좁혀
  완전 불투명(`#fff`)이 항상 이기도록 수정.
- 죽은 CSS(`.onboarding-shot`, `.shot-brand`, `.skip.float*`) 정리.

### 검증
- ✅ `npx tsc -b`, `npm run build` 통과.
- ✅ 브라우저로 스플래시·1~5단계 전체 재확인: "n/5" 배지·제목이 모든 단계에서
  픽셀 단위로 완전히 동일한 위치에 렌더링됨(더 이상 이미지에 의존하지 않으므로
  구조적으로 어긋날 수 없음).

---

## v24 업데이트 — CORS 배포 주소 교체 + 온보딩 이미지 비율 통일

> ⚠️ 이 항목의 "온보딩 이미지 비율 통일" 부분은 근본 원인 진단이 틀렸던 것으로
> 나중에 밝혀졌다. 실제 원인과 최종 수정은 위 v25 항목 참고.

### CORS
새로 배포한 프론트 주소(`kook-hemodialysis-meal-ai.vercel.app`)가 예전 팀원
배포 주소(`kook-omega.vercel.app`)를 대체했다. `server_FOOK.py`의
`CORS_ORIGINS` 기본값과 `render.yaml` 주석을 새 주소로 교체(localhost 개발
주소는 유지). 로컬에서 새 주소 허용/예전 주소 차단을 curl로 확인, 실제
Render 배포에도 반영돼 재배포 후 프론트↔백엔드 연결(회원가입→프로필
입력→식단 생성) 전체를 브라우저로 확인했다.

### 온보딩 1~5단계 이미지 비율 통일
사용자가 온보딩 단계 화면의 "n/5" 배지·제목 위치가 스텝마다 미묘하게
달라 보인다고 지적. 원인: `onboarding-1.png`~`onboarding-5.png` 각각의
제목·배지가 이미지 자체에 그려져 있는데(라이브 DOM 텍스트 아님), 5장의
가로세로 비율이 전부 달랐다(1.927 / 1.958 / 1.932 / **1.842** / 1.915).
화면은 `object-fit: cover`로 프레임 폭에 맞춰 이미지를 확대하므로, 비율이
다르면 이미지마다 확대율이 달라지고 그 안에 그려진 배지 위치가 화면에서
어긋나 보였다.

다섯 장의 배경색이 전부 거의 동일한 크림색이라는 점을 이용해, 재작업 없이
자동으로 고쳤다: 가장 큰 비율(1.958)에 맞춰 나머지 4장의 **아래쪽에만**
같은 배경색으로 캔버스를 확장(상단 제목·배지 영역은 전혀 안 건드림).
5장 모두 비율이 1.958로 통일됐고, 브라우저에서 1~5단계를 전부 다시 확인해
배지·로고 위치가 완전히 일치하는 것을 확인했다. PNG 재인코딩 과정에서
파일 용량이 다소 늘어난 것(1.5~2MB 선)은 압축 도구 차이로 보이며, 필요하면
나중에 별도로 더 최적화할 수 있다.

CSS에는 무해한 보조 개선으로 `.screen`에 `scrollbar-gutter: stable`도
추가했다(콘텐츠 길이에 따라 세로 스크롤바가 있다 없다 하면서 화면 폭이
미세하게 흔들리는 걸 방지 — 이번 문제의 근본 원인은 아니었지만 같은
증상 계열이라 같이 넣어둠).

### 검증
- ✅ `npx tsc -b --force`, `npm run build` 통과.
- ✅ 백엔드 pytest 63개 통과(무변경).
- ✅ CORS: curl로 새 주소 허용/예전 주소 차단(로컬 + 실제 Render 배포)
  확인, 실제 배포 사이트에서 회원가입→프로필→홈→식단 생성까지 클릭 확인.
- ✅ 온보딩: 로컬에서 1~5단계 전부 스크린샷 비교, "n/5" 배지 위치 완전히
  일치 확인.

---

## v23 업데이트 — PDF에 재구성 후 조리법이 들어가도록 수정

사용자가 발견: PDF 안에서 **재료**는 레버 조정 후(=실제 조정된 양) 값을 쓰는데,
**조리 과정**은 조정 전 원본(로컬 데이터) 또는 "레시피 상세 화면에서 확인하세요"
안내문구만 나와서 서로 안 맞는 문서가 만들어지고 있었다.

### 원인
`PdfPreviewPage.tsx`가 재료는 `apiResult.dish_ingredients`(서버 조정값)를 쓰면서도
조리 과정은 `menuMap`(로컬 원본)만 참조하고, 레시피 상세 화면(`RecipeBody`)이
`/recipe`로 LLM에 재구성을 요청해 받아오는 경로를 아예 타지 않았다.

### 수정
- `AppState`(`types.ts`)에 `dishSteps`(메뉴별 LLM 조리법 캐시) 추가.
- `RecipeBody.tsx`가 `/recipe` 응답을 성공적으로 받으면 이 캐시에도 저장 —
  레시피 상세 화면에서 이미 펼쳐본 메뉴는 재요청 없이 재사용된다.
- `PdfPreviewPage.tsx`는 진입 시 캐시에 없는 메뉴만(그리고 서버 조정 재료가
  있는 메뉴만) `/recipe`를 마저 호출해 채운 뒤에야 "PDF 다운로드" 버튼을
  활성화한다("레시피 준비 중..." 표시). 실패한 메뉴는 조용히 로컬 원본 →
  안내문구 순으로 폴백해서 한 메뉴 실패가 전체를 막지 않는다.
- 겸사겸사 발견한 사소한 기존 버그도 같이 고침: 로컬 데이터 중 `steps`가
  빈 배열인 메뉴(예: 백미밥)가 있었는데, `m?.steps || [...]`는 빈 배열도
  참으로 평가해서 안내문구로 안 넘어가고 빈칸으로 남았다. `RecipeBody`가
  이미 쓰던 `.length` 체크 방식으로 맞췄다.

### 검증
- ✅ `npx tsc -b --force`, `npm run build` 통과.
- ✅ 백엔드 pytest 63개 통과(무변경).
- ✅ 실제 생성 흐름(Home→...→FinalMeal→PDF)으로 진입해 확인: `/recipe`가
  메뉴별로 호출되고(네트워크 탭 확인), 성공하면 캐시에 저장되어 다운로드
  버튼이 다시 활성화됨. 로컬에 `OPENAI_API_KEY`가 없어 모든 호출이
  `{error: "..."}`로 왔는데도 페이지가 깨지지 않고 메뉴별로 로컬 데이터로
  정상 폴백하는 것 확인(그레이스풀 디그레이드 경로 검증).
- ✅ `apiResult`가 없는 오프라인 폴백 경로(직접 `/pdf` 진입)는 `/recipe`를
  아예 호출하지 않고 기존처럼 즉시 렌더 — 회귀 없음 확인.
