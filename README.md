...
동일 조건에서 반복 생성할 때 국 메뉴가 일부 메뉴로 쏠리는 현상을 발견했습니다. 원인을
추적한 결과, 디코더가 입력 seed의 국 위치 토큰을 그대로 따라가는 shortcut이 확인됐습니다.

국 슬롯을 마스킹해 재학습한 실험에서는 국 고유 종류가 **4.1개 → 28.6개**, 영양 충족률이
**68.8% → 97.6%**로 개선됐습니다. 다만 이는 동일 입력을 고정한 stress-test 결과로 실제
서비스 조건과 직접 동일시할 수 없었고, 현재 production 모델을 실제 서비스에 가까운
조건에서 재평가했을 때는 심각한 반복 현상이 확인되지 않았습니다.

따라서 가능성이 확인된 후속 개선안으로만 보존하고 현재 배포 모델에는 적용하지 않았습니다.
상세 실험 결과는
