...
| ⚙️ **개인 영양 기준 설정** | 기본 자동 기준을 유지하면서 필요 시 5대 영양소 목표를 항목별로 직접 설정. 미설정 항목은 기존 기준 유지 |
...
| ❤️ **메뉴 찜·재생성** | 생성된 식단에서 메뉴 단위로 찜하고 재료·조리과정·영양성분을 다시 확인. 찜한 메뉴를 지정해 새로운 한 끼 식단 생성 |
| 📅 **식사 기록·하루 영양 추적** | 날짜·끼니별 식사를 저장하고 달력에서 조회. 끼니별·하루 누적 영양량을 개인 기준 대비 진행률로 확인하며, 오늘 기록은 다음 한 끼의 남은 영양 기준(`consumed`)과 메뉴 중복 방지(`used_today`)에 자동 반영 |
...
이렇게 마련한 데이터를 실제로 다음 식단 생성에 연결했습니다. 현재는 오늘 기록한
식사의 섭취량을 합산해 다음 한 끼 생성 시 남은 영양 기준에 자동 반영하고, 앞서 먹은
메뉴도 다음 추천에서 가능한 한 반복되지 않도록 연결했습니다(`intake` → `consumed`,
`raw_menus` → `used_today`). 이어서 저장된 기록을 달력 기반으로 날짜별로 조회하고,
하루 누적 섭취량을 볼 수 있도록 확장했습니다. 이때 누적 섭취량 숫자만으로는 기준
대비 어느 정도인지 가늠하기 어려워, 메뉴 생성 로직 없이 신체정보만으로 개인 하루
영양 기준을 계산하는 경량 엔드포인트(`/day_targets`)를 별도로 만들어 하루 영양
진행률 표시에 사용했습니다.
...
### 4. 개인별 영양 목표를 생성·판정까지 연결

기존에는 성별·키로 계산한 영양 기준만 쓸 수 있었지만, 의료진·영양사에게 별도로
안내받은 개인 영양 목표가 있는 경우 이를 반영할 수 있도록 5대 영양소(열량·단백질·
칼륨·인·나트륨)를 항목별로 직접 설정하는 기능을 추가했습니다. 입력하지 않은 항목은
기존 자동 기준을 그대로 쓰고, 설정한 값은 `/day_targets` 표시뿐 아니라 식단 생성의
남은 영양 예산·적합성 판정·영양 보정·하루 누적 판정까지 동일한 기준으로 전달되도록
연결했습니다.

이 과정에서 나트륨을 두 개념으로 분리했습니다. 사용자에게 보여주고 개인 목표로
설정하는 값은 자연재료까지 포함한 총나트륨이고, 조미료(첨가염)를 끼니당 일정 수준
이하로 제어하는 기존 내부 기준은 별도로 유지했습니다.
...
│   ├── server_FOOK.py                           # 🚀 FastAPI 엔드포인트 (25개)
...
│   │   └── 003_custom_targets.sql               # 개인별 영양 기준 저장 컬럼 추가
│   │
...
│   │   │   ├── 📂 account/                      # 내 정보, 개인 영양 기준 설정, 식단 기록/관리/PDF 보관함, 조리 팁
│   │   │   ├── 📂 meal/                         # 홈~하루식단~생성~판정~재구성~비교~완성
│   │   │   └── 📂 recipe/                       # 레시피·찜한 메뉴 상세, PDF 미리보기
│   │   ├── 📂 components/
