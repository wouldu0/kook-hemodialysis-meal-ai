...
<details>
<summary><b>💬 추가 구현: 영양 상담 RAG 챗봇</b></summary>
...
설명만 생성합니다. 질문 두 종류를 구분해서 처리합니다.

| 질문 유형 | 처리 방식 |
|:---|:---|
| 특정 재료의 섭취 가능 여부 | **영양DB 직접 조회.** 100g당 칼륨/인/나트륨 → 1회 섭취 기준량으로 환산해 저/중/고칼륨 등급 판정. 오늘 먹은 양(`consumed`)을 같이 주면 칼륨·인은 남은 예산과 비교까지(나트륨은 재료 하나 단위로는 첨가염 예산과 비교하는 게 범주상 맞지 않아 — 첨가염 판정은 조리 시 넣는 소금·양념 기준이라 — 하드 판정 대신 정보성 주의 문구로 안내) |
| 일반 지식 질문 ("왜 위험한가요?" 등) | **RAG.** 대한신장학회 등 자료를 임베딩한 지식베이스에서 코사인 유사도로 근거를 찾아 답변. 검색된 후보가 모두 최소 유사도 기준(`RAG_MIN_SCORE`, 실측 KB 점수 분포로 도출한 기본값) 미만이면 근거 없이 LLM이 즉흥적으로 답하게 두지 않고 "확인할 수 없다"고 답함 |
...
프론트엔드는 아직 개인화(체중·오늘 섭취량 기반 예산 비교) 없이 일반 질문 모드로만 동작합니다 —
"오늘 이미 먹은 양"을 로그인 유저 기준으로 정확히 집계하는 화면/엔드포인트가 아직 없어서,
근거 없는 값을 지어내 보내는 대신 일반 질문 모드로만 연결해뒀습니다.
...
│   ├── 📂 evaluation/                           # RAG 검색 품질 평가(Top-k 적중률 · no-answer 정확도 · 라우팅 정확도)
...
│   ├── 📂 tests/                                # pytest 110개 — TF 모델 로딩 없음(RAG 챗봇 테스트는 영양DB 엑셀만 로딩)
...
│
├── 📂 .github/workflows/ci.yml                  # 백엔드 pytest 110개 + 프론트 타입체크·빌드 (TF 모델 로딩 없음)
