...
> ⚠️ 로그인 토큰이 이 주소로 전송되므로 세 겹으로 막아뒀습니다: `https://`만 허용,
> 호스트가 실제로 쓰는 호스팅 제공자 도메인(`*.onrender.com`, `*.trycloudflare.com`)일
> 때만 허용, 그리고 실제로 전환될 때만 브라우저 확인창(`confirm`)에 대상 주소를 보여준
> 뒤 동의해야 저장됩니다 — 조작된 링크를 실수로 열어도 임의의 공격자 서버로는 애초에
> 전환되지 않고, 허용된 도메인이라도 동의 없이는 자동으로 세션이 새어나가지 않습니다.
