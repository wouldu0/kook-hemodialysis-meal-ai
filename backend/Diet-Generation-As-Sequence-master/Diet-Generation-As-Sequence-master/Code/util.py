...
def get_action(preds, option):
    ...
    # stochastic policy (FOOK 추가)
    # 레포의 'target'은 확률"값"을 샘플링 후 np.where로 인덱스를 역추적해 동률일 때 엉뚱한 토큰을
    # 고른다. REINFORCE는 샘플한 토큰이 정확해야 -log π(a) 기울기가 맞으므로 인덱스를 직접 샘플링.
...
BODY_WEIGHT = 60   # 투석환자 체중(kg). 앱에서 유저 입력값으로 주입.

def get_reward_ver2(nutrient_state, done):
    '''
    투석 5영양 · 한 끼 기준 영양보상 (0~5).
    nutrient_state = get_score_pandas 결과 = [weight, Energy, Protein, Potassium, Phosphorus, Sodium] 합.
      [0]=weight합(무시) [1]=Energy [2]=Protein [3]=Potassium [4]=Phosphorus [5]=Sodium
    체중 BODY_WEIGHT(default 60) 기준 한 끼 목표 = 1일÷3. 나트륨은 끼니고정(소금 2g=786mg).
    반환: np.array([reward, done, s_E, s_prot, s_K, s_P, s_Na])  (numpy2 안전: 균일 float 배열)
    '''
    W = BODY_WEIGHT
    E    = nutrient_state[1]
    prot = nutrient_state[2]
    K    = nutrient_state[3]
    P    = nutrient_state[4]
    Na   = nutrient_state[5]

    s = np.zeros(5)
    s[0] = 1.0 if (W * 30 / 3) <= E <= (W * 35 / 3) else 0.0     # 열량
    s[1] = 1.0 if (W * 1.1 / 3) <= prot <= (W * 1.2 / 3) else 0.0  # 단백질
    s[2] = 1.0 if K <= (3000 / 3) else 0.0                       # 칼륨 상한
    s[3] = 1.0 if P <= (1000 / 3) else 0.0                       # 인 상한
    s[4] = 1.0 if Na <= (2 * 393) else 0.0                       # 나트륨 끼니고정
    nutrient_reward = float(s.sum())
    return np.array([nutrient_reward, float(done), s[0], s[1], s[2], s[3], s[4]], dtype=float)
...
# 한영번역기 (한글 전용이므로 번역표 없으면 skip — FOOK 배선)
try:
